...
def minibatch_parse(sentences, model, batch_size):
    ...
    # 这里使用python浅拷贝，它会拷贝过来的元素具有相同的地址，因此在改变这些元素的时候，原来的也会改变。
    unfinished_parses = partial_parses[:]
    while len(unfinished_parses):
        pred_trans = model.predict(unfinished_parses[:batch_size])#Take the first batch size parses in unfinished parses as a minibatch
