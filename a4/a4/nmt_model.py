...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings.source(source_padded)													# 构造x(seqlen, batchsize, embsize)
        X = pack_padded_sequence(X, source_lengths)                                                     # apply the `pack_padded_sequence` function to X
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X) 										# last_hidden / last_cell : (2, batchsize, hiddensize)
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0]												# 应用编码器后，需要将“pad_packed_sequence”函数应用于enc_hiddens(seqlen, batchsize, embsize)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)														# 置换位置(batchsize, seqlen, embsize)
