...
def naiveSoftmaxLossAndGradient(
    ...
    v_c = centerWordVec # (d,)
    # for loop method:
    # loss = -np.log(softmax(np.dot(U.T,v_c))[o])
    # gradCenterVec = sum([U[x]*softmax(np.dot(U.T,v_c))[x] for x in range(U.shape[0])]) - U[o]
    # gradOutsideVecs = np.array([v_c*(softmax(np.dot(U.T,v_c))[x]-int(x==o)) for x in range(U.shape[0])])
    # vertorization:
    y_hat = softmax(np.dot(U,v_c)) # (V,)
    y = np.zeros_like(y_hat)
    y[o] = 1
    loss = -np.log(y_hat[o])
    gradCenterVec = np.dot(U.T,(y_hat-y)) # dot((d,V),(V,))-->(d,)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        current_loss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(v_c,word2Ind[outsideWord],outsideVectors,dataset)
        loss += current_loss
