...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        doc = self.data[idx]
        truncated_len = int(torch.randint(low=4, high=int(self.block_size * 7 / 8) + 1, size=(1,))[
                                0])  # random.randint(4, int(self.block_size*7/8))
        ...
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * (
                    self.block_size - len(prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content) + 1)
